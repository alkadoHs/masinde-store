                ...
                <tbody>
                    ...
                    <?php foreach($cartItems as $item): ?>
                        <tr class="border border-slate-600">
                            ...
                            <td class=" p-3"><?= format_price($item->price) ?></td>
                            <?php $cartId = $item->cartId?>
                            <td class="px-4 py-3">
                                <input type="hidden" name="item_id[]" value="<?= $item->id ?>">
                                <input type="number" name="quantity[]" value="<?= $item->quantity ?>" id="quantity" class="bg-gray-50 border max-w-[100px] min-w-[80px] border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                            </td>
                            <?php $overAllPrice += ($item->price * $item->quantity) ?>
                            <td class=" p-3"><?= format_price($item->price * $item->quantity) ?></td>
            ...
            <ul class="flex flex-col gap-3">
                ...
                <li class="grid grid-cols-2 gap-2">
                    <?php if($overAllPrice == 0):?>
                        <button disabled data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" class="block text-white bg-blue-700/50 hover:bg-blue-800/60 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                        SELL PRODUCT
                        </button>
                    <?php else: ?>
                        <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                        SELL PRODUCT
                        </button>
                    <?php endif ?>
                            ...
                            <div class="relative bg-white rounded-lg shadow">
                                ...
                                <div class="px-6 py-6 lg:px-8">
                                    ...
                                    <input type="hidden" name="cartId" value="<?= $cartId ?>">
                                        ...
                                        <div>
                                            <label for="sellerId" class="block mb-2 text-sm font-medium text-gray-900">Seller</label>
                                            <select id="sellerId" name="sellerId" class="bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500">
