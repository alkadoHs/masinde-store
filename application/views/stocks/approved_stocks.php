  ...
  <main class="py-4 px-2 lg:px-4 md:ml-64 h-auto pt-20 ">
    <?php if($this->session->flashdata('product_not_availabele')): ?>
              <div id="toast-success" class="flex items-center w-full p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                ...
                <div class="ml-3 text-sm font-normal"><?= $this->session->flashdata('product_not_availabele') ?></div>
    ...
    <?php elseif($this->session->flashdata('complete_purchaseorder_success')): ?>
        ...
        <?php endif ?>
            ...
            <div class="max-w-screen-xl mx-auto lg:px-12 w-full">
                 <h2 class="text-white text-lg font-bold p-3 bg-emerald-900 rounded">Approved Orders</h2>
                            ...
                            <table id="productTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3">APPROVED</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $orderitem->branch ?></th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $orderitem->seller ?></th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><?= $orderitem->product_name ?></th>
                                        <td class="px-4 py-3">
                                            <input type="number" readonly name="quantity[]" value="<?= $orderitem->quantity ?>" id="quantity" class="bg-gray-50 border max-w-[100px] min-w-[80px] border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="hidden" name="order_item_id[]" value="<?= $orderitem->id ?>">
                                            <input type="hidden" name="branchId[]" value="<?= $orderitem->id ?>">
                                            <input type="hidden" name="productId[]" value="<?= $orderitem->productId ?>">
                                            <input type="hidden" name="productName[]" value="<?= $orderitem->product_name ?>">
                                            <input type="number" name="quantity2[]" value="<?= $orderitem->quantity2 ?>" id="quantity2" class="bg-gray-50 border max-w-[100px] min-w-[80px] border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                                        </td>
                                        <td class="px-3 py-2">
                                            <?php if($orderitem->quantity != $orderitem->quantity2):?>
                                                <div class="w-8 h-8 flex items-center justify-center rounded-full bg-red-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-white">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                                    </svg>
                                                </div>
                                            <?php else: ?>
                                            <div class="w-8 h-8 flex items-center justify-center rounded-full bg-green-600">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-white">
